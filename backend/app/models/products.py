...
class ProductBase(BaseIDModel):
    name: str = Field(
        description="Name of the product.", max_length=120, index=True, unique=True
    )
...
class BaseProduct(ProductBase, table=True):
    ...
    price: Optional[float] = Field(
        description="Price of the product.", ge=0.0, default=None
    )
    ...
    created_by: Optional[int] = Field(
        foreign_key="user.id", description="User ID that created the product."
    )
    last_updated_by: Optional[int] = Field(
        foreign_key="user.id", description="User ID that last updated the product."
    )
    user_updated: Optional["User"] = Relationship(
    ...
    user_created: Optional["User"] = Relationship(
        sa_relationship=RelationshipProperty(
            "User", back_populates="products", foreign_keys="[BaseProduct.created_by]"
        )
    )
